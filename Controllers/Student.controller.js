...
const createStudentInfo = async (req, res) => {
    try {
        ...
        if (!fullName || !age || !nationality || !weekdayWakeUpTime || !weekendWakeUpTime || 
            !sleepTime || cleanlinessLevel === undefined || smokes === undefined || 
            drinksAlcohol === undefined || !personalityType || petFriendly === undefined || 
            !preferredStudyEnvironment || !courseMajor || !studySchedule || 
            workingPartTime === undefined || onlineClassFrequency === undefined || 
            !weekendPreferences || okayWithSharedRoom === undefined || 
            preferredRoomTemperature === undefined || usesLightAtNight === undefined || 
            !guestFrequency) {
        ...
        if (cleanlinessLevel < 1 || cleanlinessLevel > 5) {
            return ResponseHandler.error(res, "Cleanliness level must be between 1 and 5", 400);
        }
        ...
        await StudentAuth.findByIdAndUpdate(studentId, { student: savedStudent._id });
